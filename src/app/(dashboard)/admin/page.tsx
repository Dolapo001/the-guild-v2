...
export default function AdminDashboard() {
 ...
 const handleAction = async (uid: string, action: "VERIFIED" | "REJECTED") => {
  setIsSubmitting(true);
  try {
    await adminService.verifyUser(uid, action);
 ...
 return (
  ...
  <th className="px-6 py-4 text-[10px] font-extrabold text-primary/40 uppercase tracking-widest">Email</th>
  ...
  {loading ? (
    <tr>
        <td colSpan={5} className="px-6 py-20 text-center text-foreground/40">Loading queue...</td>
    </tr>
  ) : queue.map((item) => (
  ...
  <span className="font-bold text-primary">{item.username}</span>
  </div>
  </td>
  <td className="px-6 py-4 text-sm font-bold text-foreground/60">{item.email}</td>
  ...
  <div>
  <p className="text-[10px] font-extrabold text-foreground/30 uppercase tracking-widest mb-1">Contact Email</p>
  <p className="font-bold text-primary">{selectedUser?.email}</p>
  </div>
  </div>
  ...
  onClick={() => handleAction(selectedUser?.uid!, "VERIFIED")}
  disabled={isSubmitting}
  >
  {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <><CheckCircle2 className="mr-2 h-5 w-5" /> Approve Business</>}
  </Button>
  <Button
  variant="outline"
