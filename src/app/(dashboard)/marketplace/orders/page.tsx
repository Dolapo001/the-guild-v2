...
export default function OrdersPage() {
 ...
 <div
 key={idx}
 className="h-16 w-16 rounded-xl border-2 border-white overflow-hidden relative shadow-sm"
 >
 <Image src={item.image} alt={item.name} fill className="object-cover" />
 </div>
 ))}
 {order.items.length > 3 && (
 <div className="h-16 w-16 rounded-xl border-2 border-white bg-gray-100 flex items-center justify-center text-xs font-bold text-foreground/40 shadow-sm">
 +{order.items.length - 3}
 </div>
 )}
 </div>
