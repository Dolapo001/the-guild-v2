...
export default function OrderSuccessPage() {
 ...
 <h2 className="text-xl font-extrabold text-gray-900 ">Out for Delivery</h2>
 </div>
 ...
 <div>
 <p className="text-lg font-extrabold text-gray-900 ">{order.rider.name}</p>
 ...
 onClick={() => toast.info("Live tracking is connecting...")}
 >
 ...
 <p className="font-bold text-gray-900 ">10 Admiralty Way</p>
 ...
 {order.deliveryFee ? (
 <>
 <p className="font-bold text-green-600 ">Fee Confirmed</p>
 <p className="text-[10px] font-medium text-foreground/60 mt-1">₦{order.deliveryFee.toLocaleString()} added to total.</p>
 </>
 ) : (
 <>
 <p className="font-bold text-amber-600 ">Pending Vendor Review</p>
 ...
 <GlassCard className="p-6 border-white/40 bg-white/80 backdrop-blur-2xl sticky top-28">
 <h3 className="text-lg font-extrabold text-gray-900 mb-6">Order Receipt</h3>

 <div className="space-y-4 mb-6">
 {order.items.map((item) => (
 <div key={item.id} className="flex justify-between items-start">
 <div className="flex gap-3">
 <div className="h-8 w-8 rounded bg-gray-100 flex items-center justify-center text-xs font-bold text-foreground/60">
 x{item.quantity}
 </div>
 <div>
 ...
 <div>
 <p className="text-xs font-bold text-emerald-700 ">Payment Secured</p>
 <p className="text-[10px] font-medium text-emerald-600/80 ">Funds held in escrow</p>
 </div>
 </div>
 </GlassCard>
 </div>
