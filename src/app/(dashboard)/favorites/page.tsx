...
export default function FavoritesPage() {
 ...
 const favoriteBusinesses = favorites.filter(f => f.type === 'business' || f.type === 'service');
 ...
 <div className="flex p-1 bg-white/40 rounded-xl border border-glass-border backdrop-blur-sm">
 <button
 ...
 {loading ? (
    <div className="col-span-full flex flex-col items-center justify-center">
        ...
        <p className="text-sm font-bold text-foreground/40 uppercase tracking-widest">Fetching your favorites...</p>
    </div>
 ) : activeTab === 'businesses' ? (
 filteredBusinesses.length > 0 ? (
 filteredBusinesses.map((fav) => (
 <ServiceCard key={fav.uid} service={fav.data} />
 ))
 ) : (
 ...
 <h3 className="text-xl font-bold text-gray-900 ">No saved businesses</h3>
 ...
 <ProductCard key={fav.uid} product={fav.data} />
 ))
 ) : (
 <div className="col-span-full py-20 text-center">
 <div className="h-20 w-20 rounded-full bg-primary/5 flex items-center justify-center mx-auto mb-4">
 ...
 <h3 className="text-xl font-bold text-gray-900 ">No saved products</h3>
