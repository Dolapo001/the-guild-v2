        ...
        >
          <GlassCard className="p-8 border-white/40">
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  ...
                  <div className={cn(
                    ...
                    item.status === 'Verified' ? "text-accent" : "text-amber-600"
                  )}>
                    {item.status === 'Verified' ? <CheckCircle2 className="h-4 w-4" /> : <Clock className="h-4 w-4 text-amber-500" />} {item.status}
                  </div>
                </div>
              ))}
            </div>
            <Button variant="outline" className="w-full mt-10 h-12 rounded-xl border-glass-border font-bold text-primary hover:bg-primary/5" asChild>
              <Link href="/home/profile/verification">View Documents</Link>
            </Button>
          </GlassCard>
