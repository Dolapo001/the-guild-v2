    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-mesh-gradient">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-body text-muted-foreground mt-2">Sign in to manage your business pulse.</p>
        </div>

        <GlassCard className="p-8 border-white/40 shadow-e4">
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            {mfaPending && (
              <TextField
                ...
                error={codeError}
                helper="Enter the 6-digit code sent to your email / authenticator."
                className="[&_input]:tracking-[0.4em] [&_input]:font-bold"
                autoFocus
              />
            )}
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-14 rounded-panel bg-primary text-white font-bold text-lg shadow-e3 hover:scale-[1.02] transition-transform"
            >
