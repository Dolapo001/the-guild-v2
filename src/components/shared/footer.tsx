...
export function Footer() {
  ...
  const navigation = {
    ...
    business: [
      ...
      { name: "Business Dashboard", href: "/home" },
      { name: "Verification", href: "/profile" },
  ...
  return (
    <footer className="py-24 px-6 border-t border-black/5 bg-card relative z-10">
      <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-16">
        ...
        <div>
          <h4 className="font-bold text-slate-900 mb-8 uppercase tracking-widest text-xs">For Business</h4>
          <ul className="space-y-4">
        ...
        <div className="space-y-8">
          <h4 className="font-bold text-slate-900 mb-8 uppercase tracking-widest text-xs">Download App</h4>
          <div className="space-y-3">
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={handleInstall}
              className="h-14 bg-black/5 border border-black/5 rounded-2xl flex items-center px-5 gap-4 hover:bg-black/10 transition-colors cursor-pointer group"
            >
      ...
      <div className="max-w-7xl mx-auto mt-24 pt-8 border-t border-black/5 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-8">
          {navigation.legal.map((item) => (
            <Link key={item.name} href={item.href} className="text-slate-400 hover:text-slate-900 text-xs font-bold uppercase tracking-widest transition-colors">
              {item.name}
            </Link>
          ))}
        </div>
