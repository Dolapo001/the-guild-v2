...
export function StaffProfileModal({ isOpen, onClose, staff, onBookNow }: StaffProfileModalProps) {
 ...
 className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all relative ${activeTab === "portfolio" ? "text-primary " : "text-foreground/40 hover:text-primary/60"
 }`}
 >
 Portfolio
 {activeTab === "portfolio" && (
 <motion.div layoutId="staffTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-full" />
 )}
 </button>
 <button
 onClick={() => setActiveTab("reviews")}
 className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all relative ${activeTab === "reviews" ? "text-primary " : "text-foreground/40 hover:text-primary/60"
 }`}
 >
 Reviews (124)
 {activeTab === "reviews" && (
 ...
 <div key={review.id} className="p-4 rounded-xl bg-white/40 border border-glass-border">
 ...
 <div className="p-4 border-t border-glass-border md:hidden bg-white/40 backdrop-blur-xl">
 <Button
 onClick={handleBookNow}
